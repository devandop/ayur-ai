...
async function WellnessOverview() {
  const user = await currentUser();

  // Fetch all appointments and calculate stats
  const appointments = await getUserAppointments();
  const totalAppointments = appointments?.length || 0;
  const completedAppointments = appointments?.filter((app: any) => app.status === "COMPLETED").length || 0;
  ...
  return (
    <Card className="lg:col-span-2">
      <CardHeader>
        ...
        <CardDescription>Keep track of your holistic health journey</CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-3">
                Book your first appointment or try our AI voice assistant for instant wellness advice.
              </p>
